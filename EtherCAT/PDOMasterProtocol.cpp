...
{
    for(auto i = slaves.constKeyValueBegin(); i != slaves.constKeyValueEnd(); i++)
    {
        if(!pdoSlaves.contains(i->first))
        {
            pdoSlaves.insert(i->first, new pdo_slave_t(i->second));
        }
    }
}
...
static const uint8_t CRC16_L[256] =
    {
...
uint16_t pdo_protocol_buf_t::getCRC16(uint8_t *data, uint16_t len)
{
    uint8_t crc_h = 0xFF;
    uint8_t crc_l = 0xFF;
